...
train_df['month'] = train_df['일시'].apply(lambda x: int(x[5:7]))
train_df['day'] = train_df['일시'].apply(lambda x: int(x[8:10]))
train_df['time'] = train_df['일시'].apply(lambda x: int(x[11:13]))
...
# 시계열 특성을 학습에 반영하기 위해 일시를 월, 일, 시간으로 나눕니다
test_df['month'] = test_df['일시'].apply(lambda x: int(x[5:7]))
test_df['day'] = test_df['일시'].apply(lambda x: int(x[8:10]))
test_df['time'] = test_df['일시'].apply(lambda x: int(x[11:13]))
